...
def p_program(p):
    '''program : statements'''
    # Verificar que todas las variables declaradas sean usadas
    no_usadas = declaradas - usadas
    if no_usadas:
        for var in no_usadas:
            print(f"Error semántico: Variable '{var}' declarada pero no usada.")
        sys.exit(1)
    p[0] = ('program', p[1])
...
def p_for_statement(p):
    ...
    usadas.add(p[3])  # ID usado en el for
